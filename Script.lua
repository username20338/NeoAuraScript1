...
local function createRing(radius, index)
	...
	part.Color = Color3.fromHSV(index / totalRings, 1, 1)
	part.Parent = workspace
	table.insert(rings, part)
end

for i = 1, totalRings do createRing(4, i) end
...
RunService.RenderStepped:Connect(function()
	...
	for i, ring in ipairs(rings) do
		...
		elseif mode == "SickleStar" then
			offset = Vector3.new(math.cos(theta), math.sin(theta), math.cos(theta * 2)) * 4
		elseif mode == "WitherStorm" then
			offset = Vector3.new(math.sin(theta * 1.5) * 6, math.cos(theta) * 3, math.cos(theta * 2) * 3)
		end
...
makeBtn("Mudar Modo", 50, function()
